...
namespace OperatingSystemsProject
{
    public partial class Form1 : Form
    {
        public Form1()
        {
            ...
            radioButtonSJF.CheckedChanged += new EventHandler(RadioButtonCheckChange);
        ...
        List<String> queueNames = new List<String> { "RR", "HRRN", "SRT", "SJF"};
        ...
        private void RunButtonClick(object sender, EventArgs e)
        {
            ...
            else if (radioButtonSJF.Checked)//RUN SJF
            {
                MakeCleaning();
                SJF(processList);
            }
            ...
            while (mcompleteCheck != processCount)
            {
                ...
                else
                {
                    ...
                    for (int j = 0; j < queueCount; j++)
                    {
                        ...
                        else if (queueNames[mlfqQueue[j]] == "SJF")
                        {
        ...
        private void mlfqSJF(Process p)
        {
            int currentId = p.processId - 1;
            for (int i = 0; i < burstTimeList[currentId]; i++)
            {
        ...
        #region SJF
        private void SJF(List<Process> processList)
        {
            int processCount = processList.Count;
            minimumBurstTime = int.MaxValue;
            for (int i = 0; i < processCount; i++)
            ...
            while (completeCheck != processCount)
            {
                ...
                int tempBurst = burstTimeList[currentProcess];
                ...
                if (burstTimeList[currentProcess] == 0)
                {
                    ...
                    processList[currentProcess].pTurnAroundTime = finishTime - processList[currentProcess].arrivalTime-1;
                    processList[currentProcess].pWaitingTime = processList[currentProcess].pTurnAroundTime - tempBurst;
        ...
        private void CFS(List<Process> processList, int quantum)
        {
            ...
            while (completeCheck != processCount)
            {
                ...
                else
                {
                    ...
                    quantum2 = quantum / waitingQueue.Count;
                    if (burstTimeList[currentId] > quantum2)
                    {
                        for (int i = 0; i < quantum2; i++)
                        {
                            currentTime++;
                        ...
                        burstTimeList[currentId] -= quantum2;
